import IRoleRepository from "../contracts/IRoleRepo.js";
...
class MongoRoleRepository extends IRoleRepository {
    ...
    async findOrCreateRole(name, description) {
        try {
            ...
            if (!role) {
                role = await Role.create({ name: name.toLowerCase(), description: description || `${name} role` });
            }
            return role;
        } catch (error) {
            throw new AppError("Failed to find or create role", 500);
        }
    }
