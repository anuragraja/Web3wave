...
export async function sendPasswordResetEmail({ to, name, otp }) {
    ...
    const htmlContent = `
        ...
        <body>
            <div class="container">
                ...
                <div class="content">
                    <p class="message">Hello <strong>${recipientName}</strong>,<br><br>We received a request to reset your password for your Web3Wave account. Use the following 6-digit OTP code to verify your identity and set a new password:</p>
                    <div class="otp-box">${otp}</div>
                    <p class="expiry">This verification code expires in <strong>5 minutes</strong>.</p>
                    <p class="warning">If you did not request this password reset, please ignore this email. Your existing password remains secure.</p>
                </div>
                <div class="footer">
    ...
    try {
        ...
        if (!response.ok) {
            ...
            throw new AppError("Failed to send password reset email. Please try again later.", 502);
        }
        ...
        if (error.name === "AbortError") {
            logger.error("Brevo password reset request timed out after 10000ms");
